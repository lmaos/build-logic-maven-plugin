...
通过一套简洁统一的 XML DSL 语法，即可在 Maven 构建生命周期中完成：文件操作、目录管理、字符串处理、压缩、条件判断、循环、函数、HTTP 调用等一系列自动化逻辑，无需编写 Java 代码、无需依赖外部脚本。

***设计理念***

本插件基于极简、统一、无歧义的三大核心规则设计：

name：定义变量
用于声明一个变量（普通值、文件、字符串、集合、日期等）。

ref：引用变量
引用一个已经通过 name 定义的变量。

file：引用文件变量
专用于引用 <file name="xxx"/> 定义的文件对象。
